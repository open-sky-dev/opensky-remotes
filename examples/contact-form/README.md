...
A minimal SvelteKit app showing `@opensky/remotes` (`createValidation` + `createEnhancedForm`) with a [remote form function](https://svelte.dev/docs/kit/remote-functions#form) and a valibot schema.
...
- `createValidation` — blur/input validation handlers, per-field issues, and a form-level submit handler ([src/routes/+page.svelte](src/routes/+page.svelte))
...
- `createEnhancedForm` — tracked form state (`pending`, `delayed`, `timeout`, `error`, `result`) and semantic callbacks
